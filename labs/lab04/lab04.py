def divide(quotients, divisors):
    ...
    """
    return {q: [divisor for divisor in divisors if q % divisors == 0] for q in quotients}
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1,amount//price+1):
                ...
                add([remain_fruit for remain_fruit in fruits if fruit != remain_fruit], amount - k * price, cart+display(fruit=fruit, count=k))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    reference = make_city('reference', lat=lat, lon=lon)
    return get_name(city_a) if distance(city_a, reference) < distance(city_b, reference) else get_name(city_b)
